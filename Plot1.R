...
HouseholdPower$Date_Time <- as.POSIXct(paste(HouseholdPower$Date,HouseholdPower$Time),
                                     format= "%d/%m/%Y %H:%M:%S")
...
png(filename = "./RGraphs/Plot1.png")

#open png graphics device, with file to send plot to.


hist(as.numeric(as.character(HouseholdPower1$Global_active_power)), breaks=12, col="red", 
     xlab = "Global Active Power (kilowatts)", main = "Global Active Power")

#Creates a histogram based on Global_active_power.  Since the Global_active_power is a factor column 
